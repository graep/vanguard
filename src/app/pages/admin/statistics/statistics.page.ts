// src/app/pages/admin/statistics/statistics.page.ts
  ...
  private updateCardValues(): void {
    ...
    if (this.cardsBySection['vans']) {
      this.cardsBySection['vans'].forEach(card => {
        switch(card.id) {
          ...
          case 'vans-expiring': card.value = this.vanStats.expiringDocs; break;
          case 'vans-expired': card.value = this.vanStats.expiredDocs; break;
        }
      });
    }
    if (this.cardsBySection['inspections']) {
      this.cardsBySection['inspections'].forEach(card => {
        switch(card.id) {
          case 'insp-pending': card.value = this.inspectionStats.pending; break;
          case 'insp-approved': card.value = this.inspectionStats.approved; break;
          case 'insp-rejected': card.value = this.inspectionStats.rejected; break;
          ...
          case 'insp-open-issues': card.value = this.inspectionStats.openIssues; card.label = `${this.inspectionStats.resolvedIssues} resolved`; break;
        }
      });
    }
    ...
    if (this.cardsBySection['health']) {
      this.cardsBySection['health'].forEach(card => {
        switch(card.id) {
          ...
          case 'health-attention': card.value = this.operationalHealth.needsAttention; break;
        }
      });
    }
  ...
  private createDefaultCards(): void {
    this.cardsBySection = {
      vans: [
        ...
        { id: 'vans-expiring', sectionId: 'vans', type: 'card', title: 'Expiring Documents', icon: 'warning-outline', value: this.vanStats.expiringDocs, label: 'Expiring in next 30 days', valueType: 'number', order: 3, size: '1x1' },
        { id: 'vans-expired', sectionId: 'vans', type: 'card', title: 'Expired Documents', icon: 'alert-circle-outline', value: this.vanStats.expiredDocs, label: 'Require immediate attention', valueType: 'number', order: 4, size: '1x1' }
      ],
      inspections: [
        { id: 'insp-pending', sectionId: 'inspections', type: 'card', title: 'Pending Review', icon: 'time-outline', value: this.inspectionStats.pending, label: 'Awaiting approval', valueType: 'number', order: 0, size: '1x1' },
        { id: 'insp-approved', sectionId: 'inspections', type: 'card', title: 'Approved', icon: 'checkmark-done-outline', value: this.inspectionStats.approved, label: 'Total approved', valueType: 'number', order: 1, size: '1x1' },
        { id: 'insp-rejected', sectionId: 'inspections', type: 'card', title: 'Rejected', icon: 'close-outline', value: this.inspectionStats.rejected, label: 'Total rejected', valueType: 'number', order: 2, size: '1x1' },
        { id: 'insp-today', sectionId: 'inspections', type: 'compact', title: 'Today', icon: '', value: this.inspectionStats.today, label: 'Today', valueType: 'number', order: 3, size: '1x1' },
        { id: 'insp-week', sectionId: 'inspections', type: 'compact', title: 'This Week', icon: '', value: this.inspectionStats.thisWeek, label: 'This Week', valueType: 'number', order: 4, size: '1x1' },
        { id: 'insp-month', sectionId: 'inspections', type: 'compact', title: 'This Month', icon: '', value: this.inspectionStats.thisMonth, label: 'This Month', valueType: 'number', order: 5, size: '1x1' },
        { id: 'insp-open-issues', sectionId: 'inspections', type: 'card', title: 'Open Issues', icon: 'alert-outline', iconClass: 'icon-danger', value: this.inspectionStats.openIssues, label: `${this.inspectionStats.resolvedIssues} resolved`, valueType: 'number', order: 6, size: '1x1' },
        { id: 'insp-severity', sectionId: 'inspections', type: 'card', title: 'Issue Severity', icon: 'flag-outline', value: 'custom', label: 'severity', valueType: 'custom', order: 7, size: '2x2' }
      ],
      drivers: [
      ...
      health: [
        ...
        { id: 'health-attention', sectionId: 'health', type: 'card', title: 'Needs Attention', icon: 'warning', value: this.operationalHealth.needsAttention, label: 'Grounded, open issues, or expired docs', valueType: 'number', order: 1, size: '1x1' }
      ]
    };
  }
